...
```json
{
  ...
  "entries": [
    ...
    },
    {
      "type": "action",
      "tool": "bash",
      "code": "Get-ChildItem -Path \"~/Documents\" -Filter \"*.csv\"",
    ...
    },
    {
      "type": "side-effect",
      "action": "file_write",
      "path": "~/Downloads/AGENTSON-PRD.md",
      "diff": "+187 lines"
    }
  ],
  "metadata": {
...
| `agentson export` CLI | P0 | Planned |
| `agentson search` CLI | P1 | Planned |
| `agentson render` CLI (md, html) | P1 | Planned |
...
| `agentson hydrate` command | P0 | Planned |
...
| Feature | Priority | Status |
|---------|----------|--------|
| `agentson diff` — compare sessions | P2 | Planned |
| `agentson replay` — re-feed to model | P2 | Planned |
...
agentson export --tool opencode --session ses_1069f6208ffeXozEyNyMW6RXOO

# Export all MiniMax sessions
agentson export --tool minimax --all

# Export specific Antigravity conversation
agentson export --tool antigravity --id 9a54d356-610a-4225-bcba-f0e3048cf866

# Export to specific format
agentson export --tool opencode --format json --output ./sessions/
agentson export --tool opencode --format md --output ./sessions/
```
...
agentson search "nightscout" --all

# Search specific tool
agentson search "fhir" --tool opencode

# Search with date range
agentson search "sensor" --from 2026-06-01 --to 2026-06-30

# Output as JSON
agentson search "diabetes" --format json
```
...
agentson render session.AgentSON --format html

# Render to terminal
agentson render session.AgentSON --format terminal
```
...
agentson hydrate --from opencode --to cursor --session ses_xxx

# Hydrate last session automatically
agentson hydrate --auto
...
- [ ] Release CLI (`agentson export`, `search`, `render`)
- [ ] Publish to PyPI
...
- [ ] Implement `agentson hydrate`
- [ ] Build web viewer
- [ ] Deploy Supabase managed instance
- [ ] Add `agentson push/pull` for cloud sync
...
```
~/.local/share/opencode/opencode.db     (1.6 GB, 21 tables)
~/.minimax/sqlite.db                    (22 MB, 20 tables)
~/.gemini/antigravity-ide/conversations/9a54d356-*.db  (7 MB, 7 tables)
```
