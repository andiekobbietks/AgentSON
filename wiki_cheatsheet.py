...
def build_cheatsheet():
    doc = SimpleDocTemplate(
        "C:\\Users\\LLM-Test\\Documents\\AgentSON\\wiki\\AgentSON_Wiki_Cheatsheet.pdf",
        pagesize=A4,
        rightMargin=60,
        leftMargin=60,
        topMargin=60,
    ...
    story.append(Paragraph("New-Item -ItemType Directory -Path 'C:\\Users\\LLM-Test\\Documents\\AgentSON\\wiki' -Force", code_style))
    ...
    story.append(Paragraph("Command:", bold_style))
    story.append(Paragraph("copy AgentSON\\wiki\\*.* AgentSON.wiki\\", code_style))
    story.append(Paragraph("Output:", bold_style))
    ...
    story.append(Paragraph("cd C:\\Users\\LLM-Test\\Documents<br/>git clone https://github.com/andiekobbietks/AgentSON.wiki.git<br/>copy AgentSON\\wiki\\*.* AgentSON.wiki\\<br/>cd AgentSON.wiki<br/>git add .<br/>git commit -m 'Add wiki pages'<br/>$token = Read-Host -Prompt 'Paste PAT' -AsSecureString<br/>$tokenPlain = [Runtime.InteropServices.Marshal]::PtrToStringAuto(<br/>    [Runtime.InteropServices.Marshal]::SecureStringToBSTR($token))<br/>git remote set-url origin<br/>    \"https://andiekobbietks:${tokenPlain}@github.com/andiekobbietks/AgentSON.wiki.git\"<br/>git push", code_style))

    # === PAGE 6: File Locations + Remaining Tasks ===
    story.append(PageBreak())
    story.append(Paragraph("File Locations", h1_style))
    locations = [
        "Wiki source files: ~/Documents/AgentSON/wiki/",
        "Cloned wiki repo: ~/Documents/AgentSON.wiki/",
        "Main repo: ~/Documents/AgentSON/",
        "OpenCode DB: ~/.local/share/opencode/opencode.db",
        "Cheatsheet PDF: ~/Documents/AgentSON/wiki/AgentSON_Wiki_Cheatsheet.pdf",
    ...
    doc.build(story)
    print("PDF generated: AgentSON/wiki/AgentSON_Wiki_Cheatsheet.pdf")
